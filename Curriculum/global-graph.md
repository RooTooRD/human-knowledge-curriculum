...
```mermaid
flowchart TD
    L0["L0 · Human starting point<br/>language · observation · counting · agency"]
    L1["L1 · Learning and expression<br/>reading · memory · writing · dialogue"]
    L2["L2 · Reliable reasoning<br/>logic · mathematics · evidence · computation"]
    L3["L3 · Fundamental models<br/>matter · life · mind · society · history"]
    L4["L4 · Disciplinary cores<br/>theories · methods · interpretation · design"]
    L5["L5 · Modern systems<br/>states · markets · technology · health · climate"]
    L6["L6 · Advanced analysis<br/>causality · optimization · governance · synthesis"]
    L7["L7 · Responsible action<br/>engineering · leadership · policy · creation"]
    L8["L8 · Expert inquiry<br/>research frontiers · original integration"]
...
```mermaid
flowchart LR
    FND[Foundations] --> LRN[Learning]
    FND --> WRT[Writing]
    FND --> MAT[Mathematics]
    LRN --> RSH[Research]
    LOG[Logic] --> RSH
    WRT --> RSH
    MAT --> STA[Statistics and Data]
    MAT --> PHY[Physics]
    MAT --> CSC[Computer Science]
    STA --> RSH
    CSC --> AIX[Artificial Intelligence]
    CSC --> SEC[Security]
    PHY --> CHM[Chemistry]
    PHY --> AST[Astronomy]
    CHM --> BIO[Biology]
    BIO --> HLT[Health]
    BIO --> ECS[Earth and Climate]
    RSH --> HST[History]
    RSH --> PSY[Psychology]
    RSH --> SOC[Sociology and Anthropology]
    HST --> PHI[Philosophy]
    HST --> REL[Comparative Religion]
    REL --> ISL[Islamic Studies]
    HST --> POL[Political Science]
    POL --> LAW[Law]
    STA --> ECO[Economics]
    ECO --> FIN[Finance]
    ECO --> BUS[Business]
    STA --> SYS[Systems Science]
    SYS --> ENG[Engineering]
    COM[Communication] --> BUS
    COM --> LIF[Life Skills]
    LIN[Linguistics] --> LIT[Literature]
    HST --> ART[Art]
    HST --> MUS[Music]
    ART --> ARC[Architecture and Design]
    PSY --> COG[Cognitive Science]
    LRN --> EDU[Education]
```
